...
Hold `+` (right) or `-` (left) for 0.5s. Sets a flag in `processInput` (JSL thread); the actual recenter resets `qOffset = conj(m_rawQuat)` in `RunFrame` on the SteamVR thread. Zeros `m_frozenRel`.
