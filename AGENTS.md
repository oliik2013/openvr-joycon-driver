...
Hold `+` (right) or `-` (left) for 0.5s. Sets a flag in `processInput` (JSL thread); the actual recenter resets `qOffset = yawQ * conj(m_rawQuat)` in `RunFrame` on the SteamVR thread (fresh HMD yaw). Zeros `m_frozenRel`.
